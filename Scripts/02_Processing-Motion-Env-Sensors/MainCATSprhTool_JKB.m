...
% Modified by Jessica Kendall-Bar 03.04.2022 for use with Seal Sleep Data
...
folder = 'Z://Dissertation Sleep//Sleep_Analysis//Data//PRH_Data'; % folder in the drive where the cal files are located (and where you want to look for files) %'Users//Dave//Documents//Programs//MATLAB//Tagging//CATS cal';%
...
nocam = true; %false; % set to true if this is a data only tag. If there is just audio, keep at true.  Will have to set audon independently
...
% Whale version uses this, I'd prefer to set my own. 
...
if ofs/df ~= 5; warning('Final sampling rate does not equal 25 Hz'); end
fs = ofs/df;
DV = datevec(DNorig(1));
...
CellNum = 6;
save([fileloc filename(1:end-4) 'Info.mat'],'DN','fs','ofs','CAL','camondec','tagondec','audondec','tagslipdec','CellNum','Depth','Temp','Light','inc','dec','b','-append');
disp('Section 6 done');
%% 7a.  In situ cals
...
% intervals as possible, at least
%  once per tag movement if possible,
...
% ctrl C out of find file and start after stitch audio (line 524)
% Matlab packages required: Signal Processing Toolbox
...
try 
JX = TagJiggle(A(:,1),Afs,fs,[7 12],.5,Atime+timedif/24,DN); % 10 and 90 are the high-pass and low-pass filter frequencies. The higher number will have to be < .5* Afs.
JY = TagJiggle(A(:,2),Afs,fs,[7 12],.5,Atime+timedif/24,DN);
JZ = TagJiggle(A(:,3),Afs,fs,[7 12],.5,Atime+timedif/24,DN);
J = TagJiggle(A,Afs,fs,[7 12],.5,Atime+timedif/24,DN);
Jig = [JX JY JZ J];
catch
...
minSpeed = .2; % CHANGED FOR SEALS FROM WHALE DEFAULT 0.5
...
if exist('Jig','var') && sum(isnan(Jig(:,1)))~=length(Jig(:,1))
    [~,speed,speedstats] = SpeedFromRMS3(Jig(:,1:3),'JJ',RMS2,lab,fs,Depth,pitch,roll,DN,speedper,slips,tagondec,1,0.5,minDepth,minPitch,minSpeed,.2);
...
%% 11.5 SEALS ONLY- get seal name

% See all SealIDs
whalename = filename(1:end-(18))
...
creator = 'Jessica Kendall-Bar';
notes = 'Saving seal sleep data file.';
...
CellNum = 12;
prhfile = [whaleName '_08_PRH_file' num2str(fs) 'Hzprh.mat'];

% WITHOUT SPEED
save([fileloc prhfile],'Aw','At','Gw','Gt','fs','pitch','roll','head','Depth','p','T','Light','Mt','Mw','GPS','DN','tagon','INFO');

%save([fileloc prhfile],'Aw','At','Gw','Gt','fs','pitch','roll','head','Depth','p','T','Light','Mt','Mw','GPS','DN','speed','tagon','camon','vidDN','vidNam','vidDurs','viddeploy','flownoise','INFO','audon');
%save([fileloc prhfile],'Aw','At','Gw','Gt','fs','pitch','roll','head','Depth','p','T','Light','Mt','Mw','GPS','DN','speed','JigRMS','tagon','camon','vidDN','vidNam','vidDurs','viddeploy','flownoise','INFO','audon');
%save([fileloc prhfile],'Aw','At','Gw','Gt','fs','pitch','roll','head','Depth','p','T','Light','Mt','Mw','GPS','DN','speed','speedstats','JigRMS','tagon','camon','vidDN','vidNam','vidDurs','viddeploy','flownoise','INFO','audon');


save([fileloc filename(1:end-4) 'Info.mat'],'prhfile','CellNum','INFO','-append');
disp('Section 12 finished, prh file and INFO saved');
%% 13a. SKIP FOR SEALS. Adds tag GPS data to data structure. 
...
clearvars -except prhfile fileloc filename speed tagon
load([fileloc filename(1:end-4) 'Info.mat'],'prhfile','INFO');
close all
rootDIR = strfind(fileloc,'CATS'); rootDIR = fileloc(1:rootDIR+4); % rootDIR can be used to locate the TAG GUIDE for importing further data about the tag

% NOW RUN MANUAL SPEED ESTIMATION & RETURN
...
if sum(strcmp('manual_JKB',speed.Properties.VariableNames)) % if manual speed exists
    sp = speed.manual_JKB; % use manual speed estimation
    disp('Manual JKB speed estimation used')
elseif sum(strcmp('JJ',speed.Properties.VariableNames)) % if jiggle-based speed estimate exists
    sp = speed.JJ; % use jiggle speed estimation
    disp('Jiggle-based speed estimation used')
else
    disp('Error: no speed provided')
end
...
%% 13b.5 SEALS ONLY- get seal name
...
author = 'Jessica Kendall-Bar, jkb@ucsc.edu'; % change user as appropriate
% Matlab packages required: Audio Toolbox,

save([fileloc prhfile],'geoPtrack','Ptrack','speed','sp','-append');
...
% % MODIFIED FOR SEAL RUNS
% % Steps to create netCDF files depend on tag guide setup - can uncomment and reactivate
% % lines to generate netCDF file.
...
DN2 = DN(tagon);
T = table(datestr(DN2(1:1:end),'mm/dd/yyyy HH:MM:SS'),Lats(1:1:end),Longs(1:1:end),'VariableNames',{'Time','Lat','Long'});
writetable(T,[fileloc INFO.whaleName '_08_',num2str(fs),'HzgeoPtrackLatLong.csv']);

CellNum = 13;
save([fileloc filename(1:end-4) 'Info.mat'],'CellNum','Lats','Longs','-append');
