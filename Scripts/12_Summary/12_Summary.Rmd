...
Dives_Files <- dir(here("Data",SleepEstimatesFolder),pattern=".*Dives.csv")

# Restimates_Metadata <- read_csv(here("Data","11_Sleep_Model_Summary.csv"),na='NaN')
Restimates_Metadata <- read_csv(here("Data","10_NES_AdultFemaleQC.csv"),na = c("NA","NaN", " "))
...
for (i in 1:391) { 
  ...
  if (file.exists(paste(here("Data",SleepEstimatesFolder),"/",FileIDs[i],"_Dives.csv",sep=""))){
    Dives <- read_csv(paste(here("Data",SleepEstimatesFolder),"/",FileIDs[i],"_Dives.csv",sep=""),
                      na='NaN', show_col_types = FALSE)
    Daily <- read_csv(paste(here("Data",SleepEstimatesFolder),"/",FileIDs[i],"_Daily_Activity.csv",sep=""),
                      na='NaN', show_col_types = FALSE)
    SealsUsed <- read_csv(paste(here("Data",SleepEstimatesFolder),"/",FileIDs[i],"_Seals_Used.csv",sep=""),
                          na='NaN', show_col_types = FALSE)
    ...
    QC <- Restimates_Metadata$`JKB QC`[Restimates_Metadata$TOPPID==SealsUsed$TOPPID[1]]
    if (is_empty(QC)){
      Model_good <- 0
    }else if(is.na(QC)) {
      Model_good <- 0
    ...
    if (i>1 & Model_good){
      ...
      Dives_ALL <- dplyr::bind_rows(Dives_ALL, Dives)
...
mapview(Daily_sf, map.types = "Stamen.Watercolor") 

Daily_ALL_goodtracks2 <- Daily_ALL_goodtracks %>% 
  filter(daily_recording == 24)

tracks = Daily_sf %>% group_by(TOPPID) %>% 
    summarise(do_union=FALSE) %>% st_cast("LINESTRING")


ggplot() + geom_sf(data=Daily_sf,aes(col=TOPPID)) 


ggplot() + geom_sf(data=tracks,aes(col=TOPPID))

write.csv(Daily_ALL_goodtracks,here("Data","11_Restimates_ALL_DailyActivity_ALL_goodtracks.csv"),row.names = FALSE)


NewRaw_SLEEP_downsample <- NewRaw_SLEEP %>% 
  slice(floor(seq(1, nrow(.), length.out = 10000)))
NewRaw_sf <- st_as_sf(NewRaw_SLEEP, coords = c("Long", "Lat"),  crs = 4326)
mapview(NewRaw_sf, map.types = "Stamen.Watercolor") 
```
...
# Duration (in minutes) of the dive with the longest nap:
longest_oceannap_divedur_min <- Dives_SLEEP$Duration_s[Dives_SLEEP$is_sleep==max(Dives_SLEEP$is_sleep)]/60
# Duration of the longest dive (with or without sleep):
longest_oceandive_min <- max(Dives_SLEEP$Duration_s)/60

# Duration (in minutes) of the longest dive with sleep:
longest_oceannapdive_min <- max(Dives_SLEEP$Duration_s[Dives_SLEEP$is_sleep!=0])/60
# Duration (in minutes) of the shortest dive with sleep:
shortest_oceannapdive_min <- min(Dives_SLEEP$Duration_s[Dives_SLEEP$is_sleep!=0])/60
# Mean duration (in minutes) of dives with sleep:
mean_oceannapdive_min <- mean(Dives_SLEEP$Duration_s[Dives_SLEEP$is_sleep!=0])/60
# SD of dive duration (in minutes) of dives with sleep:
sd_oceannapdive_min <- sd(Dives_SLEEP$Duration_s[Dives_SLEEP$is_sleep!=0])/60
...
paste("Their breath-holding capacity liberates a niche where they can safely sleep at depth, during short (<", 
      ...
      "min) naps during dives up to",longest_oceannapdive_min,"minutes long and up to", max_sleepdepth_m, 
