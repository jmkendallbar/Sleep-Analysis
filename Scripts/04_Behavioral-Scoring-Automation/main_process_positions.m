...
data_stack_raw = table2array(Table(:,[36,37,38])); % Just pitch and roll.
...
label_stack_str = Table.Body_position;
label_stack_cat = categorical(Table.Body_position);
...
Settings.target_label = {label_id([1,2,3,5,6,7])}; 
Settings.label_name = {label_names([1,2,3,5,6,7])}; 
% Or manually (e.g.):
...
grid on
legend(label_names([1,2,3,5,6,7]))
