...
%data_stack_raw = table2array(Table(:,[23,24,25])); % Just acc.
%data_stack_raw = table2array(Table(:,[22,34,36,37,38])); % Pressure, ODBA, prh
data_stack_raw = table2array(Table(:,[22,23,24,25,34])); % Pressure, AccXYZ, ODBA
...
% new class 1: GALUMPHING
label_map(7) = 1; % Mapping label {'7-Galumphing'}

% new class 2: SWIMMING
...
% new class 4: VISIBLY_BREATHING
label_map(1) = 4; % Mapping label {'1-Visibly Breathing'}

% new class 5: VISIBLY_BREATHING_UW
label_map(15) = 5;% Mapping label {'15-Bobbing (Up & Down)'}
label_map(17) = 5;% Mapping label {'17-Logging'}

% new class 6: NOT_VISIBLY_BREATHING
label_map(2) = 6; % Mapping label {'2-Not Visibly Breathing'}

% new class 7: NOT_VISIBLY_BREATHING_UW
label_map(11) = 7;% Mapping label {'11-Laying at Bottom of Pool'}
label_map(14) = 7;% Mapping label {'14-Drifting (Motionless)'}
label_map(16) = 7;% Mapping label {'16-Drift Logging'}
label_map(18) = 7;% Mapping label {'18-Bobbing (Motionless)'}
label_map(19) = 7;% Mapping label {'19-Still Bobbing'}
label_map(20) = 7;% Mapping label {'20-Drift Bobbing'}
...
label_names = {{'1-Galumphing'}, {'2-Swimming'}, {'3-Calm_Behaviors'}, {'4-Visibly_Breathing'}, {'5-Visibly_Breathing_UW'}, {'6-Not_Visibly_Breathing'}, {'7-Not_Visibly_Breathing_UW'}};
...
%sample_freq = 25; % [Hz]
window_len = 10; % [s] TRYING LONGER WINDOW
...
% Settings.target_label = {label_id(1:7)}; 
% Settings.label_name = {label_names(1:7)}; 
% % Or manually (e.g.):
...
% Manually define the interested label IDs and corresponding names.
Settings.target_label = {label_id(1:7)};
Settings.label_names = {label_names(1:7)};
